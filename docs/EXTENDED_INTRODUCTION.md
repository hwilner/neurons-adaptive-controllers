...
where $u_t$ is the total knob adjustment, $e_t$ is the current discomfort, $\sum_{\tau=0}^{t} e_\tau$ is the accumulated history of discomfort (the running-total column), $e_t - e_{t-1}$ is how fast the discomfort is changing, and $k_p, k_i, k_d$ are the three knobs-on-knobs saying how much weight each reaction gets.
...
where $a$ is your action (your move), $d$ is the disturbance (the opponent's move), the squared bracket is the cost — squared so sign cancels and big misses count extra — and $\min_a \max_d$ reads "choose the move whose worst-case outcome is as good as possible." *What this buys you:* robustness thinking — design against an adversary, not an average day. *What it costs you:* real disturbances are not strategic opponents, so worst-case reasoning can be needlessly conservative.
...
where $T^*$ is the resting value ("the temperature the knob stops changing"), and the algebra says: at a resting value the correction term must be zero, and since the gain $g$ is not zero, the error must be zero — the system can only rest exactly at the setpoint. *What this buys you:* stability becomes a concrete question — does iterating from a nearby value walk toward the fixed point or away? *What it costs you:* finding the fixed point does not by itself prove every start converges; a gain of 2 has the same fixed point and never settles.
...
where the numerator counts visits to band $B$ and dividing by the total turns counts into fractions — the setpoint band wins because the loop keeps pushing the trajectory back into it, raising its count. *What this buys you:* an interpretation of stability as *population*, robust to the fact that no single step is exactly at the setpoint. *What it costs you:* it is a static snapshot — it cannot distinguish a loop that settled from one that oscillates tightly inside the same bin.
...
where $d_t$ is how far the point sits from the target, and $|1-g|$ is the shrink factor per step — less than 1 and the point walks in; equal to 1 and it paces forever; more than 1 and it flies off. *What this buys you:* a single picture that separates "fast/slow" from "direct/oscillating." *What it costs you:* one-dimensional pictures do not prepare you for coupled systems whose distance can shrink in one direction while growing in another.
...
where $S$ is the safe set, the symbols $\exists\, N \;\forall\, t \ge N$ read "there exists a step $N$ such that for every later step $t$," and $e_t \in S$ means the error is a member of the tolerated set — "eventually inside, never out again." *What this buys you:* specifications — you can *state* a requirement (enter the set, never leave) and check it row by row. *What it costs you:* membership is binary; the lens hides how comfortably inside the set you are.
...
where $\hat{x}_{t+1}$ is the predicted next value, $x_{t-j}$ is the value $j$ steps back, $w_j$ is how much that lag counts (the mixing proportion), and $b$ is the bias — a constant offset, the "baseline warmth" added regardless of history. *What this buys you:* the entire model is inspectable — print the table and read it. *What it costs you:* a table of weights cannot express "if the value exceeds 5, ignore it"; the lens is limited to smooth mixing.
...
where $y_t$ is the true next value, $\hat{y}_t$ the predicted one, and $\min_{w,b}$ reads "choose the weights and bias that make this total smallest."
...
- **Error signal** (`error_signal = test_targets - predictions`): subtract what was predicted from what happened — the same subtraction as the shower table, used here only as a score: $e_t = y_t - \hat{y}_t$, where $e_t$ is the miss at step $t$.
...
where the numerator is the model's total squared miss, the denominator is the total squared miss of the lazy "always predict the mean $\bar{y}$" strategy, and subtracting from 1 turns "fraction of the laziness you still have" into "fraction you eliminated."

- **PID update** (`PIDController._simulate`): the prediction is `setpoint + kp·error + ki·(running sum of errors) + kd·(error now minus error a moment ago)` — the shower table with three columns added, exactly the formula $u_t = k_p e_t + k_i \sum_\tau e_\tau + k_d (e_t - e_{t-1})$ from section 2.
- **Gain grid search** (`PIDController.fit`): try every combination of `kp` from 0–10 and `ki`, `kd` from 0–1 (6 values each) and keep the combination with the lowest training error: $(k_p^*, k_i^*, k_d^*) = \arg\min \sum_t e_t^2$ over the grid, where $\arg\min$ reads "the grid point at which the total squared error is smallest."
...
where $\sigma$ is the width of the bump (larger $\sigma$ means more neighbors count), the exponential makes weights fall off smoothly with distance $k$, and dividing by $\sum_k w_k$ makes the weights add to one so the average does not change the overall level.
...
where $|w_j|$ is the size of the weight on lag $j$, the logarithm turns exponential fade into a straight line, and the slope $\lambda$ is the fade rate — "how many questions of memory are lost per step back."
...
where each factor in the numerator says "were both above (or both below) their own averages at step $t$," and the denominator divides out the scales so $\rho$ lands between −1 and 1 — 1 means the two series rise and fall in lockstep.
